...
- **`invert_signal` for HL perps/manual (#774)** — new `StrategyConfig.InvertSignal bool` field (`invert_signal`): flips BUY↔SELL on non-zero signals from `runHyperliquidCheck` before execution. Lets inverse-trend variants reuse the same open/close strategy refs without forking the Python module. HOLD (0) never flipped. Validation rejects `invert_signal=true` with `direction="short"` (double-flip) and on non-HL-perps/manual strategies. SIGHUP-blocked while positions are open. Default off; opt in by setting `invert_signal: true`.
...
| Invert signal | `invert_signal` | HL perps/manual only. `true` flips BUY↔SELL on every non-zero signal; HOLD (0) never flipped. Allows inverse-trend use of any open strategy without a new Python module. Rejected with `direction="short"` (double-flip). SIGHUP-blocked while open. Default `false`. |
